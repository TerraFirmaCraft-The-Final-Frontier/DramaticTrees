...
public class TreeBuilder {
	 ...
	 * REQUIRED
	 * <p>
	 * Each tree is given a unique name per mod.  We use a {@link ResourceLocation} to get the job done as it contains
	 * the ModId and the Name of the object.
	 *
	 ...
	 * responsibility to assign and maintain an ordered set of numbers that each represent a tree.  The sequence should
	 * start from 0 for each mod and incremented for each {@link TreeFamily} that the mod creates.  Gaps in the numbered
	 * list are okay(if a tree is removed for instance), duplicates will result in undefined behavior. DynamicTrees
	 * internally maintains a mapping of {@link BlockDynamicLeaves} for each mod.  This is done to reduce the number of
	 * registered blocks.
	 *
	 * @param seqNum The registration sequence number for this MODID. Used for registering 4 leaves types per {@link
	 *               BlockDynamicLeaves}.
	 * @return TreeBuilder for chaining
	 */
	 ...
	 * @param subBlockNum The number used to select the correct subblock of the {@link BlockDynamicLeaves} (typically
	 *                    0-3 for a standard {@link BlockDynamicLeaves})
	 * @return
	 */
	 ...
	 *
	 * @param primLeaves The primitive leaves which are used for many purposes including drops, branch reinforcing and
	 *                   some other basic behavior.
	 * @return TreeBuilder for chaining
	 */
	 ...
	 * OPTIONAL
	 * <p>
	 * Sets the cellular automata kit to use for the {@link BlockDynamicLeaves} for this {@link TreeFamily}
	 * <p>
	 ...
	 * OPTIONAL
	 * <p>
	 * Provides a way to inject a custom common species.  If this is not used a default Species will be created for
	 * you.
	 *
	 ...
	 * OPTIONAL
	 * <p>
	 * Can be used to turn off automatic seed creation. When enabled a seed will automatically be created and a standard
	 * seed dropper will be added to the common species.
	 *
